...
// Package the exact worktree that just passed all build and browser checks.
// Create an unattached Git-data commit; final ref promotion is separate.
const expected = process.env.GITHUB_SHA;
const repo = process.env.GITHUB_REPOSITORY;
const token = process.env.GITHUB_TOKEN;
const branch = process.env.GITHUB_REF_NAME;
if (!token || !/^[0-9a-f]{40}$/.test(expected || '') ||
    repo !== 'IssisX/Sunder-3D-GTA-like' || branch !== 'ChatGPT-B-showcase') {
...
const expectedHashes = {
  'src/game/body-render-core.ts': 'f0d8f4aab5516a48e1dbed2ecdcbdbf91c7f6f92fb7ee19a9fb62607d1b0c470',
  'src/game/body-appearance.ts': 'efea9731a3920535bb326d2796af7a4ebb4dbf7eebc0d1918009c46802744534',
  'src/game/soundscape.ts': '886f119363c2d40120ee184cd21d1927964916c4e56f787be618bced7dd17c90',
};
...
for (const [path, content] of source) {
  ...
  if (expectedHashes[path] && digest !== expectedHashes[path]) {
    throw new Error(`Validated source hash changed: ${path}`);
  }
