...
import { weaponEnds } from "./physique";
import { MESH_REV, makeHumanoid, syncHumanoid, isBodyGeo, rebuildHeldWeapon, placeHeldWeapon } from "./humanoid";
...
export class View {
  ...
  hemi = new THREE.HemisphereLight(0xb8c4d0, 0x3a3024, 0.95);
  amb = new THREE.AmbientLight(0x2a241c, 0.64);
  ...
  dispose() {
    ...
    this.scene.traverse((o) => {
      ...
      if (m.geometry && m.geometry !== GEO.box && m.geometry !== GEO.sphere && m.geometry !== GEO.cyl && m.geometry !== GEO.cone && m.geometry !== GEO.plane && !isBodyGeo(m.geometry)) {
        m.geometry.dispose();
      }
  ...
  private updateFire(w: World) {
    ...
    this.fill.position.set(p.x - f.x * 2.4, p.y + 2.05, p.z - f.z * 2.4);
    if (fires[0]) {
      this.fill.intensity = Math.max(2.8, Math.min(4.4, fires.length * 0.75));
    } else if (p.torchLit) {
      this.fill.intensity = 3.1;
    } else {
      this.fill.intensity = 3.2;
    }
  }
  ...
  private updateCamera(w: World, cam: Cam, alpha: number, title: boolean) {
    ...
    if (title) {
      ...
      this.fill.position.set(
        this.look.x + (cx - this.look.x) * 0.22,
        3.4,
        this.look.z + (cz - this.look.z) * 0.22,
      );
      this.fill.distance = 24;
      this.fill.intensity = Math.max(this.fill.intensity, 6.2);
    } else {
      this.fill.distance = 14;
    }
  }
