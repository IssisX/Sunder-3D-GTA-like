...
export const EDGES = { localIncidentRoles: true, guardEvidence: true };
...
export class SocialIncidentController {
  ...
  // A witnessed incident is evidence for investigation, not permission for
  // combat. Only an independently authorized guard may join as a fighter.
  private guardCanJoin(w: World, a: Actor, atk: Actor) {
    if (!EDGES.guardEvidence || a.faction !== "guard") return true;
    if (a.lastHitBy === atk.id && w.time - a.lastHitT < 12) return true;
    if (a.ai !== "combat" || a.targetId !== atk.id ||
      w.time - a.lastSeenT >= 8) return false;
    for (const m of a.memories) {
      if (m.kind === "threat" && m.who === atk.id &&
        w.time - m.t < 8 && m.certainty > 0.35) return true;
    }
    return false;
  }
  private assignHumanWitnesses(w: World, slot: number, atk: Actor, vic: Actor) {
    this.candidateId.fill(0);
    this.candidateScore.fill(-Infinity);
    let socialCandidates = 0;
    let localGuards = 0;
    for (const o of w.actors) {
      if (!o.alive || o.kind === "player" ||
        o.species !== "human" || o.id === atk.id ||
        o.id === vic.id) continue;
      const dx = this.centerX[slot]! - o.x;
      const dz = this.centerZ[slot]! - o.z;
      const d = Math.hypot(dx, dz);
      if (d > HUMAN_OBSERVE_RADIUS ||
        !this.canObserve(w, o, this.centerX[slot]!,
          this.centerZ[slot]!, d)) continue;
      // The ordinary guard AI retains ownership of warning/investigation.
      if (!this.guardCanJoin(w, o, atk)) continue;
      prepareAgent(w, o);
      const sameFaction = vic.faction !== "none" &&
        o.faction === vic.faction;
      const guardDuty = o.faction === "guard" &&
        atk.faction !== "guard";
      const homeD = Math.hypot(o.homeX - vic.homeX,
        o.homeZ - vic.homeZ);
      const neighborhood = Math.exp(-homeD * 0.17);
      if (sameFaction && neighborhood > 0.18) socialCandidates++;
      if (guardDuty) localGuards++;
      if (sameFaction || guardDuty) {
        const score = (sameFaction ? 0.12 : 0) +
          (guardDuty ? 0.36 : 0) + o.loyalty * 0.26 +
          o.courage * 0.18 + o.aggression * 0.22 +
          neighborhood * 0.2 - o.fear * 0.22 - d * 0.025 +
          agentRandom(w, o) * 0.08;
        if (score > 0.52) this.insertCandidate(o.id, score);
      }
    }
    const roll = hash01((w.seed >>> 0) ^
      Math.imul(atk.id + 17, 0x9e3779b1) ^
      Math.imul(vic.id + 31, 0x85ebca6b) ^ this.serial[slot]!);
    let joinCap = 0;
    if (socialCandidates >= 1 && roll < 0.42) joinCap = 1;
    if (socialCandidates >= 3 && roll < 0.18) joinCap = 2;
    if (socialCandidates >= 5 && roll < 0.045)
      joinCap = Math.min(MAX_JOINERS, socialCandidates);
    if (localGuards > 0)
      joinCap = Math.max(joinCap, Math.min(3, localGuards));
    let joined = 0;
    for (let i = 0; i < MAX_JOINERS && joined < joinCap; i++) {
      const id = this.candidateId[i]!;
      if (!id) continue;
      const o = w.actor(id);
      if (!o?.alive || !this.guardCanJoin(w, o, atk)) continue;
      this.assignFighter(w, slot, o, atk.id);
      joined++;
    }
    for (const o of w.actors) {
      if (!o.alive || o.kind === "player" ||
        o.species !== "human" || o.id === atk.id ||
        o.id === vic.id || o.id < 0 || o.id >= ENTITY_CAP ||
        this.role[o.id] !== 0) continue;
      const dx = this.centerX[slot]! - o.x;
      const dz = this.centerZ[slot]! - o.z;
      const d = Math.hypot(dx, dz);
      if (d > HUMAN_OBSERVE_RADIUS ||
        !this.canObserve(w, o, this.centerX[slot]!,
          this.centerZ[slot]!, d)) continue;
      if (o.faction === "guard") continue;
      prepareAgent(w, o);
      const drive = 0.42 + o.courage * 0.34 -
        o.fear * 0.42 - this.intensity[slot]! * 0.12 +
        agentRandom(w, o) * 0.16;
      if (drive > 0.44) this.assignSpectator(w, slot, o);
      else this.assignLeave(w, slot, o);
    }
  }
