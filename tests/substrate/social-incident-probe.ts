...
try {
  ...
  const uninvolved = roleCount(many.w, SOCIAL_ROLE.FIGHTER);
  assert.equal(uninvolved, 1,
    "witness guards bypassed the canonical warning/evidence authority");
  assert.equal(socialIncidents.roleOf(many.allyA.id), SOCIAL_ROLE.NONE);
  assert.equal(socialIncidents.roleOf(many.allyB.id), SOCIAL_ROLE.NONE);

  // Independently established direct harm may authorize a local group.
  socialIncidents.reset();
  for (const ally of [many.allyA, many.allyB]) {
    ally.lastHitBy = many.player.id;
    ally.lastHitT = many.w.time;
  }
  socialIncidents.reportAggression(many.w, many.player, many.victim, "kick", 0.55);
  const smallGroupFighters = roleCount(many.w, SOCIAL_ROLE.FIGHTER);
  assert.equal(smallGroupFighters, 3,
    "independently harmed local guards lost their ability to join");
  ...
  socialIncidents.reset();
  EDGES.guardEvidence = false;
  const unrestricted = smallPileOnWorld();
  socialIncidents.reportAggression(unrestricted.w, unrestricted.player,
    unrestricted.victim, "kick", 0.55);
  assert.equal(roleCount(unrestricted.w, SOCIAL_ROLE.FIGHTER), 3,
    "cutting guardEvidence did not restore automatic enlistment");
  EDGES.guardEvidence = true;
  ...
  console.log("PASS local social incident roles", { singleFighters, singleSpectators, chantEvents: chantEvents.length, uninvolved, smallGroupFighters, cutRoles });
} finally {
  EDGES.localIncidentRoles = true;
  EDGES.guardEvidence = true;
  socialIncidents.reset();
